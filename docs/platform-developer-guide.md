...
Guide for contributing to the ripple-next monorepo — from a bare Mac to running, testing, and deploying the platform.

> **Building an app with `@ripple-next/*` packages?** You're in the wrong guide.
> See the **[Consumer App Guide](./consumer-app-guide.md)** instead.
...
## 1A. Platform developer workflow
...
If you are building an app *with* `@ripple-next/*` packages (not modifying the
platform itself), see the **[Consumer App Guide](./consumer-app-guide.md)** for
complete setup instructions covering frontend-only, backend-only, and full-stack
workflows.
